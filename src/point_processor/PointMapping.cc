...
namespace lio {
	...
	void PointMapping::SetupRos(ros::NodeHandle &nh, bool enable_sub) {
		...
		if (enable_sub) {
			...
			if (compact_data_) {
				sub_compact_data_ =
						nh.subscribe<sensor_msgs::PointCloud2>("/compact_data", 2, &PointMapping::CompactDataHandler,
						                                       this);
			} else {
				...
				if (point_search_sq_dis[num_neighbors - 1] < min_match_sq_dis_) {
					...
					for (int j = 0; j < num_neighbors; j++) {
						if (fabs(pa * laser_cloud_surf_from_map_->points[point_search_idx[j]].x +
						         ...
						         pc * laser_cloud_surf_from_map_->points[point_search_idx[j]].z + pd) >
						    min_plane_dis_) {
							planeValid = false;
							break;
						}
	...
	void PointMapping::UpdateMapDatabase(lio::PointCloudPtr margin_corner_stack_downsampled,
		...
		for (int i = 0; i < margin_valid_size; ++i) {
			...
			float center_z =
					50.0f * (last_k - margin_cube_center.laser_cloud_cen_height); // NOTE: center of the margin cube
	...
	void PointMapping::PublishResults() {
		...
		if (map_frame_count_ >= num_map_frames_) {
			...
			PublishCloudMsg(pub_laser_cloud_surround_, *laser_cloud_surround_downsampled_, time_laser_odometry_,
			                "/camera_init");
		}
